...
let canche_of_rain = document.getElementById("chance-of-rain")
let feelslike = document.getElementById("feelslike")
let canche_of_snow = document.getElementById("chance-of-snow")
...
window.addEventListener('DOMContentLoaded', () => {
    fetch('https://ipinfo.io/json?token=1b4628ce6ebce0') // você pode usar sem token com limitações
...
function fetchWeatherData(city) {
    ...
    const apiUrl = `http://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${city}&days=7&aqi=yes&alerts=yes&lang=pt`;
...
function displayWeatherData(data) {
    ...
    canche_of_rain.innerText = `${data.forecast.forecastday[0].day.daily_chance_of_rain}%`
    feelslike.innerText = `${data.current.feelslike_c} °C`
    canche_of_snow.innerText = `${data.forecast.forecastday[0].day.daily_chance_of_snow}%`
